...
Each widget in the tree has an `Element`. The element owns the position in the
tree, and that position retains the `State` object. When a rebuild puts the
same widget type and key at the same position, Noir updates the configuration
and keeps the state. Change the type, the key, or the position, and Noir
disposes the old state before mounting a new one.
...
`deferDispose` needs the unreleased Noir alpha.5. See
...
The repository's
[Pulse animation](https://github.com/conceptadev/noir/blob/main/example/pulse_animation.dart)
