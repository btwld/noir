...
per-request budget flag. A long-lived session can accumulate more than that
across requests, so embedding applications must also bound their message count.
