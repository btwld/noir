...
`noir_signals` and its required Noir 0.0.1 are not on pub.dev yet, so a
checkout is the only way to resolve them today. You will build inside that
checkout. To add the same packages to an application of your own instead, use
the [install instructions](../README.md).
