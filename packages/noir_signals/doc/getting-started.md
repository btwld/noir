...
`noir_signals` is not on pub.dev yet and requires Noir 0.0.2. You will build
inside the checkout, which resolves both packages from the same revision. To add the same packages to an application of your own instead, use
