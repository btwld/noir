...
run `wait key primitive:calculate` through `noir_driver:drive`:
...
### 5. `noir_driver:drive` cannot send Ctrl with a digit or Ctrl+Enter

`encodeKey` accepts the named keys plus `ctrl-<a-z>`
(`packages/noir_driver/lib/src/ansi_keys.dart`). A keyboard contract that uses
