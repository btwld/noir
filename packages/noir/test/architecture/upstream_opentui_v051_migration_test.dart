...
void main() {
  test('pins canonical OpenTUI v0.5.1 without the legacy Go package', () {
    ...
    expect(
      Directory('../../external/opentui/packages/go').existsSync(),
      isFalse,
    );
  });
  ...
  test('asset tool is explicit and fail-closed', () {
    final source = File('tool/fetch_opentui_binaries.dart').readAsStringSync();
    ...
    final result = Process.runSync('dart', [
      'run',
      'tool/fetch_opentui_binaries.dart',
    ]);
  ...
  test('fork-only build and Go parity systems are absent', () {
    for (final path in <String>[
      'tool/build_opentui_candidates.dart',
      'tool/native_build',
      'tool/run_go_snapshot.sh',
