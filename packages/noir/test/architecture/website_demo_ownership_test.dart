...
void main() {
  test(
    ...
    () {
      final recipe =
          jsonDecode(File('tool/recordings/counter.json').readAsStringSync())
  ...
  test('every published frame is a capture of a shipped checkpoint', () {
    ...
    for (final scene in scenes) {
      ...
      expect(
        ...
        reason:
            'scene $id is stale. Run `dart run tool/capture_doc_frames.dart`.',
      );
  ...
  test('every published screenshot names the checkpoint it came from', () {
    ...
    const imageRoot = '../noir_signals/doc/images';
